...
class DiceCELoss(nn.Module):
    ...
    This is a common and robust loss for semantic segmentation.
    """
    def __init__(self, ce_weight=1.0, dice_weight=1.0, smooth=1e-6):
        ...
        self.smooth = smooth  # Smoothing factor to avoid division by zero in Dice
        
    def forward(self, logits, targets):
        """
        Args:
            logits (torch.Tensor): Model output of shape (N, C, D, H, W).
            targets (torch.Tensor): Ground truth of shape (N, 1, D, H, W).
        """
        # --- Cross Entropy Loss ---
        # CrossEntropyLoss expects target of shape (N, D, H, W) and type long
        ...
        # Target shape: (N, 1, D, H, W) -> squeeze -> (N, D, H, W)
        # one_hot -> (N, D, H, W, C)
        # permute -> (N, C, D, H, W) to match probs shape
        ...
        # Calculate Dice score for each class and each batch item
...
def calculate_dice_per_class(logits, targets, num_classes, smooth=1e-6):
    ...
    # One-hot encode predictions: (N, D, H, W) -> (N, D, H, W, C) -> (N, C, D, H, W)
    preds_onehot = F.one_hot(preds, num_classes).permute(0, 4, 1, 2, 3)
    # One-hot encode targets: (N, 1, D, H, W) -> (N, D, H, W) -> ... -> (N, C, D, H, W)
    targets_onehot = F.one_hot(targets.squeeze(1).long(), num_classes).permute(0, 4, 1, 2, 3)
    ...
    return dice_score.cpu().numpy() # Return as numpy array
...
def train(args):
    ...
    # Get the final average per-class scores
